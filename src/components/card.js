...
export default function MainCard() {
  ...
  const addWishList = (dat) => {
    if (Object.keys(prof).length > 0) {
      if (fav.some((f) => f.titleAr === dat.titleAr)) {
        setfav(fav.filter((f) => f.titleAr !== dat.titleAr));
      } else {
  ...
  useEffect(() => {
    if (categoryy === "Design"||categoryy === "تصميم") {
      setLast(data);
    } else {
    ...
    <>
      <div className="container d-flex">
        <div className="row ">
          {lastDes.map((dat, ky) => (
            <Card
              className={`col-lg-3 col-md-4 col-sm-6 col-xs-12 border-0 `}
              key={ky}
            >
              <IconButton
                size="large"
                onClick={() => addWishList(dat)}
                sx={{
                  width: 40,
                  position: "absolute",
                  top: 10,
              ...
              <Carousel className="parent" interval={50000}>
                <Carousel.Item>
                  <img
                ...
                <Card.Body className="text-muted">
                  <div className="d-flex flex-row justify-content-between text-black">
                    {currentLanguageCode === "ar" ? (
                      <Card.Title onClick={() => toDetail(dat)}>
                        {dat.addressAr}
                      </Card.Title>
                    ) : (
                      <Card.Title onClick={() => toDetail(dat)}>
